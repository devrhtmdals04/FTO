...
mod tests {
    ...
    fn test_xt_mirroring() {
        ...
        assert!(
            (xt_home - xt_away).abs() < 1e-6,
            "xT scores should be mirrored for Home and Away teams"
        );
        ...
        assert!(
            (xt_home - xt_home_right).abs() > 1e-6,
            "xT scores should be different for the same team on opposite sides"
        );
    }
}
