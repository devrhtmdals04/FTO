use super::tactics_view::{PhaseFocus, QuantifiedTactics};
use crate::ai::formations::{layout_from_directive, PhaseLayout};
use crate::ai::phase::TeamPhase;
use crate::ai::{PlayerId, Role};
...
fn apply_kickoff_rules(positions: &mut [Vec2; N_PER_TEAM], team: TeamId, attacking: bool) {
    ...
    if attacking {
        positions[10] = Vec2::ZERO;
        positions[9] = Vec2::new(side * 9.0, 4.0);
    } else {
...
#[derive(Clone, Debug)]
pub enum CoachCmd {
    SetLineWidth { line: f32, width: f32, compact: f32 },
    SetPress { intensity: f32 },
    SetRoleOverride { player: PlayerId, role: Role },
}
