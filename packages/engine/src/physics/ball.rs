use crate::params::{
    DT, E_Z, FRICTION_V_REF, G, MU_AIR, MU_BOUNCE, MU_GROUND_KINETIC, MU_GROUND_STATIC, PITCH_H,
    PITCH_W, VZ_MIN,
};
...
fn step_ground_ball(world: &mut World) {
    ...
    if speed > 0.0 {
        ...
        let mu_dynamic =
            MU_GROUND_KINETIC + (MU_GROUND_STATIC - MU_GROUND_KINETIC) * friction_factor;
...
fn step_air_ball(world: &mut World, grid: &SpatialHash, rng: &mut Pcg32) {
    crate::physics::interaction::handle_aerial_interactions(world, grid, rng);
