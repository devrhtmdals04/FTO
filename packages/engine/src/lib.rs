...
impl WasmEngine {
    ...
    pub fn list_tactics(&self) -> Result<JsValue, JsValue> {
        let storage =
            Self::local_storage().ok_or_else(|| JsValue::from_str("localStorage not available"))?;
        let tactics_json = storage
            .get_item("fto_tactics")
    ...
    pub fn load_tactic(&self, id: &str) -> Result<JsValue, JsValue> {
        ...
        let tactics_json = storage
            ...
            .map_err(|_| JsValue::from_str("Failed to get item from localStorage"))?
            .unwrap_or_else(|| "{}".to_string());
        let tactics: std::collections::HashMap<String, UiTactic> =
            serde_json::from_str(&tactics_json).unwrap_or_default();
    ...
    pub fn save_tactic(&mut self, tactic_json: JsValue) -> Result<(), JsValue> {
        let tactic: UiTactic = serde_wasm_bindgen::from_value(tactic_json)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
        let storage =
            Self::local_storage().ok_or_else(|| JsValue::from_str("localStorage not available"))?;
        let mut tactics: std::collections::HashMap<String, UiTactic> = storage
            .get_item("fto_tactics")
            .map_err(|_| JsValue::from_str("Failed to get item from localStorage"))?
        ...
        let new_tactics_json =
            serde_json::to_string(&tactics).map_err(|e| JsValue::from_str(&e.to_string()))?;
        storage
            .set_item("fto_tactics", &new_tactics_json)
            .map_err(|_| JsValue::from_str("Failed to set item in localStorage"))?;
